...
for j = 1:numel(xx)
    xj = xx(j);
    [rrr, ttt] = meshgrid([r2 ; r ; r1], [t ; 2*pi]);
    [xxx, yyy] = pol2cart(ttt, rrr);
    h1 = pcolor(xxx, yyy, u{j}); hold on
    caxis(gca, [0 1])
    set(h1, 'ZData', xj + 0*get(h1, 'ZData'));
    tmp = get(h1, 'XData');
...
xx = linspace(0, 1, 100);
th = .9*pi/2;
plot3(xx, 3*cos(th)+0*xx, 3*sin(th)+0*xx, 'k', 'LineWidth', 3);
plot3(xx, cos(th)+0*xx, sin(th)+0*xx, ':k', 'LineWidth', 3);
th = .95*3*pi/2;
plot3(xx, 3*cos(th)+0*xx, 3*sin(th)+0*xx, 'k', 'LineWidth', 3);
plot3(xx, cos(th)+0*xx, sin(th)+0*xx, ':k', 'LineWidth', 3);
...
pause(1)
% print -dpng ../paper/figures/cyl_plot1
